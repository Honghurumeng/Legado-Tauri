...
pub async fn search(source: &LegadoBookSource, keyword: &str, page: u32) -> Result<Vec<BookItem>> {
    ...
    let html = http::fetch_text(&url, Some(&source.book_source_url)).await?;
    let doc = parse_document(&html);
    let ctx = ContextNode::Document(&doc);
    let nodes = apply_rule_nodes(&ctx, &source.rule_search.book_list, &url)?;
    let mut items = Vec::new();
    for node in nodes {
        let name = first(&node, &source.rule_search.name, &url)?;
        let book_url = first(&node, &source.rule_search.book_url, &url)?;
        if name.is_empty() || book_url.is_empty() {
            continue;
        }
        items.push(BookItem {
            name,
            author: first(&node, &source.rule_search.author, &url)?,
            book_url: resolve_url(&url, &book_url),
            cover_url: opt(first(&node, &source.rule_search.cover_url, &url)?)
                .map(|value| resolve_url(&url, &value)),
            last_chapter: opt(first(&node, &source.rule_search.last_chapter, &url)?),
            latest_chapter: None,
            latest_chapter_url: None,
            word_count: opt(first(&node, &source.rule_search.word_count, &url)?),
            chapter_count: None,
            update_time: opt(first(&node, &source.rule_search.update_time, &url)?),
            status: None,
            kind: opt(first(&node, &source.rule_search.kind, &url)?),
            intro: opt(first(&node, &source.rule_search.intro, &url)?),
        });
    }
    Ok(items)
}
...
pub async fn book_info(source: &LegadoBookSource, book_url: &str) -> Result<BookDetail> {
    ...
    let doc = parse_document(&html);
    let ctx = ContextNode::Document(&doc);
    let toc_url = first(&ctx, &source.rule_book_info.toc_url, &url)?;
    let name = first(&ctx, &source.rule_book_info.name, &url)?;
    Ok(BookDetail {
        name,
        author: first(&ctx, &source.rule_book_info.author, &url)?,
        book_url: url.clone(),
        cover_url: opt(first(&ctx, &source.rule_book_info.cover_url, &url)?)
            .map(|value| resolve_url(&url, &value)),
        intro: opt(first(&ctx, &source.rule_book_info.intro, &url)?),
        kind: opt(first(&ctx, &source.rule_book_info.kind, &url)?),
        last_chapter: opt(first(&ctx, &source.rule_book_info.last_chapter, &url)?),
        latest_chapter: None,
        latest_chapter_url: None,
        word_count: opt(first(&ctx, &source.rule_book_info.word_count, &url)?),
        chapter_count: None,
        update_time: opt(first(&ctx, &source.rule_book_info.update_time, &url)?),
        status: None,
        toc_url: Some(if toc_url.is_empty() {
            url
        } else {
            resolve_url(&url, &toc_url)
        }),
    })
}
...
pub async fn chapter_list(source: &LegadoBookSource, book_url: &str) -> Result<Vec<ChapterItem>> {
    ...
    loop {
        ...
        let doc = parse_document(&html);
        let ctx = ContextNode::Document(&doc);
        let nodes = apply_rule_nodes(&ctx, &source.rule_toc.chapter_list, &url)?;
        for node in nodes {
            let name = first(&node, &source.rule_toc.chapter_name, &url)?;
            let chapter_url = first(&node, &source.rule_toc.chapter_url, &url)?;
            if name.is_empty() || chapter_url.is_empty() {
                continue;
            }
            let vip = if !source.rule_toc.vip.is_empty() {
                Some(!first(&node, &source.rule_toc.vip, &url)?.is_empty())
            } else if !source.rule_toc.is_vip.is_empty() {
                Some(!first(&node, &source.rule_toc.is_vip, &url)?.is_empty())
            } else {
                None
            };
            chapters.push(ChapterItem {
                name,
                url: resolve_url(&url, &chapter_url),
                group: None,
                vip,
                price: None,
                currency: None,
        ...
        let next = first(&ctx, &source.rule_toc.next_toc_url, &url)?;
        if next.is_empty() {
            break;
        }
        let next_url = resolve_url(&url, &next);
        if next_url == url {
            break;
        }
        url = next_url;
...
pub async fn chapter_content(source: &LegadoBookSource, chapter_url: &str) -> Result<String> {
    ...
    loop {
        ...
        let doc = parse_document(&html);
        let ctx = ContextNode::Document(&doc);
        let content = first(&ctx, &source.rule_content.content, &url)?;
        if !content.is_empty() {
            pieces.push(clean_content_html(&content));
        }
        let next = first(&ctx, &source.rule_content.next_content_url, &url)?;
        if next.is_empty() {
            break;
        }
        let next_url = resolve_url(&url, &next);
        if next_url == url || !is_same_chapter_page(&url, &next_url) {
            break;
        }
        url = next_url;
    }
